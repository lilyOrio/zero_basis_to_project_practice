...
public class DrawRelativeLayout extends RelativeLayout {
    ...
    public DrawRelativeLayout(Context context, AttributeSet attrs) {
        ...
        mPaint.setStrokeWidth(mStrokeWidth);//设置画笔线宽
        mPaint.setStyle(Paint.Style.STROKE);//设置画笔类型，STROKE表示空心，FILL表示实心
    }
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        int width = getMeasuredWidth();//获取布局实际宽度
        int height = getMeasuredHeight();//获取布局实际高度
        if (width > 0 && height > 0) {
            if (mDrawType == 1){//绘制矩形
                Rect rect = new Rect(0, 0, width, height);
                ...
                canvas.drawCircle(width/2, height/2, radius, mPaint); // 在画布上绘制圆角矩形
